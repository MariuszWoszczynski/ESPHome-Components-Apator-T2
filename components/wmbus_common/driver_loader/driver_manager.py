...
class DriverManager:
    ...
    def sync_to_directory(self, target_dir: str | Path) -> None:
        ...
        for driver in sorted(self._requested_drivers, key=lambda item: item.name):
            ...
            anchor = re.sub(r"[^a-zA-Z0-9_]", "_", driver.name)
            anchor_name = f"wmbus_driver_anchor_{anchor}"
            new_content = (
                driver.serialize()
                + f'\nextern "C" bool {anchor_name}() {{ return ok; }}\n'
            )
        ...
        registry_path = target_dir / "_requested_driver_registry.cpp"
        declarations = "\n".join(f'extern "C" bool {name}();' for name in anchors)
        calls = " && ".join(f"{name}()" for name in anchors) or "true"
        registry_content = f"""// Generated by wmbus_common. Do not edit.
{declarations}

namespace esphome {{
namespace wmbus_common {{
bool force_link_requested_drivers() {{ return {calls}; }}
}}  // namespace wmbus_common
}}  // namespace esphome
"""
        if not registry_path.exists() or registry_path.read_text() != registry_content:
            registry_path.write_text(registry_content)
        written_files.add(registry_path)
